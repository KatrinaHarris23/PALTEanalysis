#this analysis is done off of the B2 population resurected from freezer stocks at days 17, 44, 66, and 90. They are isolated both in planktonic conditions and after 1 additional day of biofilm selection. 

#samples were sequenced on the MiGS 190828 run. 
...
B2_comparison <- read.csv("/Users/katrina/Desktop/scripts/B2_comparison_standard_Breseq_Output.csv", header=TRUE, stringsAsFactors = F)
head(B2_comparison) #SeqID column is the one that I want
nrow(B2_comparison) #5344
...
B2_comparison_filtered <- B2_comparison[!(B2_comparison$SeqID %in% ancestor_snps$SeqID),]
nrow(B2_comparison_filtered)#3427

#write the file to a folder to have for later use
write.csv(B2_comparison_filtered, "/Users/katrina/Desktop/B2_comparison/B2_comparison_filtered.csv")
...
#inport the data
filtering <- read.csv("/Users/katrina/Desktop/B2_comparison/B2_comparison_filtered.csv", stringsAsFactors = F)
...
B_17 <- filtering[(filtering$Sample == "B2_17B_standard"),]
B_44 <- filtering[(filtering$Sample == "B2_44B_standard"),]
B_66 <- filtering[(filtering$Sample == "B2_66B_standard"),]
B_90 <- filtering[(filtering$Sample == "B2_90B_standard"),]

P_17 <- filtering[(filtering$Sample == "B2_17P_standard"),]
P_44 <- filtering[(filtering$Sample == "B2_44P_standard"),]
P_66 <- filtering[(filtering$Sample == "B2_66P_standard"),]
P_90 <- filtering[(filtering$Sample == "B2_90P_standard"),]
...
#reorder the columns
plank_col_order <- c("0","B2_17P_standard","B2_44P_standard","B2_66P_standard","B2_90P_standard") #the order I want the columns to be
...
#reorder the columns
biof_col_order <- c("0","B2_17B_standard","B2_44B_standard","B2_66B_standard","B2_90B_standard") #the order I want the columns to be
...
write.csv(planktonic_frequencies, "/Users/katrina/Desktop/B2_comparison/planktonic_frequencies.csv")
write.csv(biofilm_frequencies, "/Users/katrina/Desktop/B2_comparison/biofilm_frequencies.csv")

plank_freqs <- read.csv("/Users/katrina/Desktop/B2_comparison/planktonic_frequencies.csv", stringsAsFactors = F)
biof_freqs <- read.csv("/Users/katrina/Desktop/B2_comparison/biofilm_frequencies.csv", stringsAsFactors = F)
...
filter3_p <- subset(filter2_p, filter2_p$B2_17P_standard < 95)
...
filter3_b <- subset(filter2_b, filter2_b$B2_17B_standard < 95)
#filter 4!!! remove if the mutation doesn't change in frequency by at least 10% 
...
#just seeing where we lose mutations
nrow(biof_freqs)#1237
nrow(filter1_b)#800
nrow(filter2_b)#368 I have no idea why there are so many mutations seen at only one time point.
nrow(filter3_b)#356
nrow(filter4_b)#356

#now save these filtered files: 
write.csv(filter4_p, "/Users/katrina/Desktop/B2_comparison/plankotnic_filtered.csv")
write.csv(filter4_b, "/Users/katrina/Desktop/B2_comparison/biofilm_filtered.csv")
...
#and write the file
write.csv(cast_filtered_17, "/Users/katrina/Desktop/B2_comparison/day17_filtered.csv")
...
#and write the file
write.csv(cast_filtered_44, "/Users/katrina/Desktop/B2_comparison/day44_filtered.csv")
...
#and write the file
write.csv(cast_filtered_66, "/Users/katrina/Desktop/B2_comparison/day66_filtered.csv")
...
#and write the file
write.csv(cast_filtered_90, "/Users/katrina/Desktop/B2_comparison/day90_filtered.csv")
...
d17 <- read.csv("/Users/katrina/Desktop/B2_comparison/day17_filtered.csv", stringsAsFactors = F)
d44 <- read.csv("/Users/katrina/Desktop/B2_comparison/day44_filtered.csv", stringsAsFactors = F)
d66 <- read.csv("/Users/katrina/Desktop/B2_comparison/day66_filtered.csv", stringsAsFactors = F)
d90 <- read.csv("/Users/katrina/Desktop/B2_comparison/day90_filtered.csv", stringsAsFactors = F)
...
cor.test(~ Planktonic + Biofilm,
         ...
         conf.level = 0.95)
#Pearson for Day 17
#Pearson's product-moment correlation: t = 15.593, df = 113, p-value < 2.2e-16
...
cor.test(~ Planktonic + Biofilm,
         ...
         conf.level = 0.95)
#Day 44
#Pearson's product-moment correlation: t = 32.819, df = 139, p-value < 2.2e-16
alternative hypothesis: true correlation is not equal to 0
95 percent confidence interval:
  0.9187398 0.9574676
sample estimates:
...
cor.test(~ Planktonic + Biofilm,
         ...
         conf.level = 0.95)
#Day 66 
#Pearson's product-moment correlation: t = 43.095, df = 147, p-value < 2.2e-16
alternative hypothesis: true correlation is not equal to 0
95 percent confidence interval:
  0.9486732 0.9728410
sample estimates:
...
cor.test(~ Planktonic + Biofilm,
         ...
         continuity = F, 
         conf.level = 0.95)
#data:  Planktonic and Biofilm t = 62.622, df = 157, p-value < 2.2e-16
alternative hypothesis: true correlation is not equal to 0
95 percent confidence interval:
  0.9734929 0.9857619
sample estimates:
