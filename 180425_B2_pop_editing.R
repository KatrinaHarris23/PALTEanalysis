...
B2_filter4 <- (subset(B2_filter3, B2_filter3$Sums < 400))
nrow(B2_filter4) #846
B2_filter4_out <- (subset(B2_filter3, B2_filter3$Sums >= 400))
...
View(B2_split5)
B2_Muller <- B2_split5[,c(1:7)]
B2_Muller$Population <- "B2"
B2_Muller$Population2 <- 1 #make sure this is a number
B2_Muller$Chromosome <- 1 #make sure this is a number
B2_Muller$Position <- B2_split5$Position
B2_Muller$Class <- "SNP"
B2_Muller$Mutation <- B2_split5$Mutation
B2_Muller$Gene <- ""
B2_Muller$AminoAcid <- ""
B2_Muller$Class2 <- ""
B2_Muller$Amino <- ""
...
write.xlsx(B2_Muller_try, file="B2_Muller.xlsx", row.names = FALSE)
